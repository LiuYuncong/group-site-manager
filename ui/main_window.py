...
class MainWindow(ctk.CTk):
    ...
    def _after_settings_saved(self):
        ...
        self.git_manager = GitManager(self.config.repo_path)
        # 【核心修复】：将 config.json 里的 URL 同步到真实的 .git/config 中
        if self.config.remote_git_url:
            self.git_manager.set_remote_url(self.config.remote_git_url)
    ...
    def pull_updates(self):
        ...
        def task():
            try:
                ...
                # 捕获任何意外崩溃，防止主界面永远锁死
                logger.exception("拉取线程发生崩溃")
                success, msg = False, f"拉取时发生系统异常：{e}"

...
class GitManager:
    ...
    # 修改 1：在 GitManager 类中新增 set_remote_url 方法
    def set_remote_url(self, url: str, remote_name: str = 'origin') -> bool:
        """将 UI 中配置的远程 URL 真正写入底层 Git 仓库配置中"""
        if not self.is_valid() or not url:
            return False
        try:
            # 如果 origin 已经存在，则更新 url；如果不存在，则创建
            if remote_name in [r.name for r in self.repo.remotes]:
                self.repo.remote(name=remote_name).set_url(url)
            else:
                self.repo.create_remote(remote_name, url)
            logger.info(f"底层 Git 远程仓库 {remote_name} 地址已同步为: {url}")
            return True
        except Exception as e:
            logger.error(f"同步 Git 远程地址失败: {e}")
            return False
    ...
    def pull_latest(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            # 【修复点】：加上错误日志，防止默默失败
            logger.error("未找到 origin 远程仓库")
            return False, "未配置远程仓库（origin），请先在设置中填写并保存。"

        # 3. 执行 pull
        try:
            # 1. 自动获取当前活动分支名称 (等同于你手动输入的 'main')
            current_branch = self.repo.active_branch.name
            
            # 2. 修复环境变量覆盖 Bug：继承原有系统环境，只追加 SSH 配置
            import os
            custom_env = os.environ.copy() 
            custom_env["GIT_SSH_COMMAND"] = "ssh -o StrictHostKeyChecking=accept-new -o BatchMode=yes"
            
            # 3. 明确指定拉取的远端和分支，等同于执行 `git pull origin <当前分支>`
            pull_info = origin.pull(current_branch, env=custom_env)
            
            logger.info(f"Pull 成功: {pull_info}")
            return True, "已成功从远程仓库拉取最新更改。"
            
        except GitCommandError as e:
            # 4. 提取详细错误日志并进行脱敏 (防范凭证泄露)
            import re
            raw_error = e.stderr if e.stderr else str(e)
            safe_error = re.sub(r'://[^@]+@', '://***@', raw_error)
            logger.error(f"Pull 失败详细原因: \n{safe_error}")
            
            # 尝试根据错误信息返回友好的中文提示
            stderr = safe_error.lower()
            if "conflict" in stderr:
                return False, "拉取时发生冲突，请手动解决冲突或联系管理员。"
            elif "authentication" in stderr or "auth" in stderr or "permission denied" in stderr:
                return False, "远程仓库认证失败，请检查 SSH 密钥配置或网络连接。"
            elif "could not resolve host" in stderr or "connection refused" in stderr:
                return False, "无法连接到远程仓库，请检查网络是否通畅。"
            elif "no tracking information" in stderr or "set-upstream" in stderr:
                return False, f"当前分支未关联远程分支。请在终端执行: git branch --set-upstream-to=origin/{current_branch} {current_branch}"
            else:
                return False, f"拉取失败：{safe_error}"

...
repo_path 必须指向一个已存在的目录（可以是空的，但 Git 功能需要该目录为 Git 仓库）。
...
管理内容
浏览列表：点击左侧模块，右侧会列出所有现有条目（显示标题和日期）。

新建条目：点击顶部 ➕ 新建 按钮，填写标题、日期、正文，可选择封面/头像图片，最后点击「保存」。

编辑条目：在列表中点击条目右侧的「编辑」按钮，修改信息后保存。
...
本地预览
确保系统已安装 Hugo，然后点击 🌐 本地预览，程序会自动运行 hugo server -D 并在默认浏览器中打开预览地址。
...
🙏 致谢
感谢 Hugo 和 Blox 提供的优秀静态站点框架。

UI 基于 CustomTkinter 构建。
